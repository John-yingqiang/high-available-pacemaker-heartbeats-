...
LOG_FILENAME = "/var/log/usx-vmg-move.log"
...
def find_vhd(path, vhd_file):
    ...
    """
    vhd_path = '' 
    ret = run_cmd('find %s -maxdepth 1 -name %s' % (path, vhd_file))
    if ret['stderr'] == '':
        vhd_path = ret['stdout'][:-1]
        #vhd_path = ret['stdout']
    else:
        raise Exception('No vhd file of %s found, detail as %s' % (vhd_file, ret['stderr']))
    debug('vhd path is %s' % vhd_path)
    return vhd_path
...
#                continue
            for item in vm_item['files']:
                search_dir = '%s/USX_%s' % (volume_info['dedupfsmountpoint'], vm_item['vmuuid']) 
                src_file_path = find_vhd(search_dir, item['vmfile'])
                dst_file_path = '%s/%s' % (target_floder, item['targetfilename'])

                if not src_file_path:
                    debug('File of %s was not found' % item['vmfile'])
                    return_item_json['success'] = False
                    return_item_json['message'] = 'File of %s was not found' % item['vmfile']
                    return_list.append(return_item_json)
                    ret_val = 1
                    continue
                #src_file_path = list(set(src_file_path))
                #for src_path_t in src_file_path:
                #    if not os.path.exists(src_path_t):
                #        debug('File of %s was not found' % src_path_t)
                #        return_item_json['success'] = False
                #        return_item_json['message'] = 'File of %s was not found' % src_path_t
                #        return_list.append(return_item_json)
                #        ret_val = 1
                #        continue
