...
class DiskUsageCollector(diamond.service_collector.ServiceCollector):
    ...
    def collect(self):
        ...
        dedup_dev = self.get_dedup_devname()
        ...
        self.log.error('Disk statistics time delta:%s' % time_delta)
        self.log.error('Disk statistics:%s' % results)
        if not results:
        ...
        for key, info in results.iteritems():
            ...
            self.log.error('Disk statistics final:%s' % metrics)
                ...
                for metkey in metrics:
                    ...
                    if metkey == 'reads':
                        ...
                        if disk_dev in metric_name:
                            ...
                            if dedup_dev and dedup_dev in metrics_name:
                                newname = string.replace(metrics_name, dedup_dev, 'usxdedup', 2)
                                newpath = string.replace(mypath, dedup_dev, 'usxdedup', 2)
                                self.publish(newname, value, newpath, None, 2, imprecise_metric=True)
                    if metkey == 'writes':
                    ...
                    if metkey == 'service_time':
                        ...
                        if disk_dev in metric_name:
                            ...
                            mypath = string.replace(self.get_metric_path(metric_name), self.ilio_uuid, resource_id, 1) # replace ILIO UUID with resource ID
                            self.publish(metric_name, metrics[metkey], mypath, None, 2, imprecise_metric=True)
                            if dedup_dev and dedup_dev in metric_name: # for Hybrid volume, report usxdedup service time
                                myname = string.replace(metric_name, dedup_dev, 'usxdedup', 2)
                                mypath = string.replace(self.get_metric_path(metric_name), dedup_dev, 'usxdedup', 2) # replace metrics path key word for latency dashboard display
                    ...
                    if metkey == 'read_byte_per_second' or metkey == 'write_byte_per_second':
                        partition_names = metric_name.rpartition('.')
                        if metkey == 'read_byte_per_second':
                            metric_name = partition_names[0] + ".reads." + partition_names[-1]
                        elif metkey == 'write_byte_per_second':
                            metric_name = partition_names[0] + ".writes." + partition_names[-1]
                        if disk_dev == 'xvda': metric_name = metric_name.replace('xvd', 'sd')
                        if disk_dev in metric_name:
                            self.publish(metric_name, metrics[metkey], self.get_metric_path(metric_name), None, 2, imprecise_metric=True)
                        else:
        ...
        if adjusted_servtime: # non-Hybrid volume, gather service time average from all ibds
            for rkey, rvalue in serv_time_dict.iteritems():
                ...
                self.publish("usxdedup.usxdedup.service_time", adjusted_service_time, adjusted_servtime[rkey], None, 2, imprecise_metric=True)
